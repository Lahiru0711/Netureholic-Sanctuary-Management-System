...
export default class B1_Report extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ReportData: [],
  ...
  componentDidMount() {
    axios.get("/Bungalow_1/").then((response) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {this.state?.ReportData?.map((p, index) => {
                return (
                  <TableRow key={index}>
                    <TableCell align="left">{p.B1_pID}</TableCell>
                    <TableCell align="right">{p.B1_pName}</TableCell>
                    <TableCell align="right">{p.B1_quantity}</TableCell>
                    <TableCell align="right">Rs.{p.B1_unitPrice}</TableCell>
                    <TableCell align="right">Rs.{p.B1_unitPrice * p.B1_quantity}</TableCell>
                    <TableCell align="right">{p.B1_demand}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
